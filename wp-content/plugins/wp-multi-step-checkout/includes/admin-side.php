...
//            array($this, 'admin_contents')
    ...
    public function admin_settings_page() {
        ...
        switch( $tab ) {
            case 'titles' : 
                ...
                break;
            case 'general' : 
                $form_label_class = 'col-sm-7 control-label';
                $general_settings = array(
                    'main_color', 'show_back_to_cart_button', 'show_shipping_step', 'unite_billing_shipping', 'unite_order_payment', 'keyboard_nav',    
                );
                break;
        }
    ...
    /**
     * Output the Zoom Settings page
     */
    public function zoom_settings_page( $allowed_tabs, $tab ) {
        $valid_license = get_option( 'gallery_zoom_license_status' ) != 'valid' ? false : true;

        // Get the messages
        $messages = $this->show_messages();

        $general_settings = array('enableZoom', 'lensShape', 'cursor', 'easingAmount', 'ratio', 'lensText', 'enableMobile', 'lensBorderSize', 'lensBorderColour', 'lensFade', 'tint', 'tintColor', 'tintOpacity', 'zoomWindowWidth', 'zoomWindowHeight', 'responsiveThreshold', 'zoomWindowPosition', 'zoomWindowOffsetx', 'borderSize', 'zoomWindowShadow', 'borderColour', 'borderRadius', 'scrollZoom', 'zwFade'); 

        // All settings
        $settings = $this->get_settings();
        if ( !$valid_license ) {
            foreach($settings as $_key => $_value ) {
                $settings[$_key]['disabled'] = true;
            }
        }

        // Get the saved options
        $settings_values = get_option('gz_options');

        // Save the options
        if ( ! empty( $_POST ) ) {
            check_admin_referer('gzoom_'. $tab);
            $new_values = $this->validate( $_POST, $general_settings );
            if ( $settings_values == false ) {
                $settings_values = $new_values;
            } else {
                $settings_values = array_merge( $settings_values, $new_values );
            }
            update_option( 'gz_options', $settings_values );
            self::add_message( 'success', '<b>'.__('Your settings have been saved.') . '</b>' );
            $messages = $this->show_messages();
        }
        ...
        // Show the options
        require_once GZ_PLUGIN_PATH . 'includes/forms-helper.php';
        $forms_helper = new GalleryZoom_FormsHelper;
        ob_start();
        include_once GZ_PLUGIN_PATH . 'includes/admin-zoom-settings-template.php';
        $contents = ob_get_contents(); ob_end_clean();

        // Show the page
        //$messages = '';
        include_once GZ_PLUGIN_PATH . 'includes/admin-template.php';
        echo str_replace('{$content}', $contents, $template);
